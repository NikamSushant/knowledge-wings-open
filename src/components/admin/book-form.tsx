...
import { uploadPdfFromUrl } from "@/lib/books.functions";
...
export type BookFormSubmit = Omit<BookFormValues, "id">;
...
export function BookForm({
  ...
  onSubmit: (values: BookFormSubmit) => Promise<void>;
}) {
  ...
  const [existingPdfPath] = useState(initial?.pdfPath ?? null);
  const [existingCoverPath] = useState(initial?.coverPath ?? null);
  ...
  async function handle(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const categorySlug = String(fd.get("category") || "ambedkar-thought");
      await onSubmit({
        slug,
        title: titleVal,
        ...
        categorySlug,
        category:
          categories.find((c) => c.slug === categorySlug)?.name ?? "Ambedkar Thought",
  ...
  return (
    <form onSubmit={handle} className="space-y-6">
      <Card title="Book details">
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Title" required>
            <input name="title" required defaultValue={initial?.title ?? ""} className="input" placeholder="e.g. The Life of Babasaheb" />
          </Field>
          <Field label="Subtitle">
            <input name="subtitle" defaultValue={initial?.subtitle ?? ""} className="input" placeholder="Optional" />
          </Field>
          <Field label="Slug (URL)">
            <input name="slug" defaultValue={initial?.slug ?? ""} className="input" placeholder="auto from title" />
          </Field>
          <Field label="Copyright year" required>
            <input name="year" required type="number" defaultValue={initial?.year ?? new Date().getFullYear()} className="input" />
          </Field>
          <Field label="Language" required>
            <select name="language" required defaultValue={initial?.language ?? "English"} className="input">
              {languages.map((l) => <option key={l}>{l}</option>)}
            </select>
          </Field>
          <Field label="Category" required>
            <select name="category" required defaultValue={initial?.categorySlug ?? "ambedkar-thought"} className="input">
              {categories.map((c) => <option key={c.slug} value={c.slug}>{c.name}</option>)}
            </select>
          </Field>
          <Field label="Tags (comma-separated)">
            <input name="tags" defaultValue={initial?.tags?.join(", ") ?? ""} className="input" placeholder="ambedkar, constitution, youth" />
          </Field>
          <Field label="Publish status" required>
            <select name="status" required defaultValue={initial?.status ?? "draft"} className="input">
              <option value="draft">Draft</option>
              <option value="published">Published</option>
            </select>
          </Field>
          <Field label="Pages">
            <input name="pages" type="number" min={0} defaultValue={initial?.pages ?? 0} className="input" />
          </Field>
        </div>
        <div className="mt-4">
        ...
        <div className="mt-4">
          <Field label="A note from the author">
            <textarea name="authorNote" rows={3} defaultValue={initial?.authorNote ?? ""} className="input" />
          </Field>
        </div>
      </Card>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Cover image">
            {currentCoverUrl && !coverPreview && (
              <img src={currentCoverUrl} alt="current cover" className="mb-2 h-32 w-24 rounded object-cover shadow" />
            )}
            <input
              type="file"
              accept="image/*"
            ...
            />
            {coverPreview && <img src={coverPreview} alt="preview" className="mt-2 h-32 w-24 rounded object-cover shadow" />}
          </Field>
          <Field label={existingPdfPath ? "Replace PDF (optional)" : "PDF (optional)"}>
            {existingPdfPath && <div className="mb-2 text-xs text-muted-foreground">Current PDF is attached. Uploading a new file replaces it.</div>}
            ...
            {pdfSource === "upload" ? (
              ...
              <>
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  The server downloads the file and stores it in your library (bypasses browser CORS).
                </div>
              </>
            )}
          </Field>
          <Field label="Allow PDF download">
            <select name="allowPdfDownload" className="input" defaultValue={initial?.allowPdfDownload ? "yes" : "no"}>
      ...
      <div className="flex flex-wrap items-center justify-end gap-3">
        ...
        <button type="submit" disabled={busy} className="btn-cta inline-flex items-center gap-2 rounded-md px-5 py-2.5 text-sm font-bold disabled:opacity-60">
  ...
  return (
    <label className="block">
      <span className="mb-1 block text-xs font-bold uppercase tracking-wider text-muted-foreground">
        {label}
        {required && <span className="text-[color:var(--color-cta)]"> *</span>}
      </span>
      {children}
    </label>
  );
